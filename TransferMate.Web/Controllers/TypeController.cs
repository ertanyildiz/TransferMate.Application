...
namespace TransferMate.Web.Controllers
{
    ...
    public class TypeController : ControllerBase
    {
        ...
        public TypeController(IConfiguration configuration, ILogger<TypeController> log)
        {
        ...
        public IActionResult Select(int id)
        {
            try
            {
                ...
                var typeRepository = new TypeRepository(_configuration.GetConnectionString("DefaultConnection"));
                var taskList = typeRepository.Select(id);
                return Ok(taskList);
            }
            catch (Exception ex)
            {
